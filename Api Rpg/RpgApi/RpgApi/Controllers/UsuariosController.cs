...
namespace RpgApi.Controllers
{
    ...
    public class UsuariosController : ControllerBase
    {
        ...
        //I.D para acessar as chaves de uma arquivo json =)
        ...
        public async Task<IActionResult> AutenticarUsuario(Usuario credenciais)
        {
            try
            {
                ...
                else
                {
                    ...
                    //Criar um token para o usuário que está logando no momento
                    usuario.token = CriarToken(usuario);

                    //Retonando as informações do usuário, e vai retornar o token do usuário =)
                    return Ok(usuario);
                }
            }
            catch (System.Exception ex)
        ...
        private string CriarToken(Usuario usuario)
        {
            ...
            //Informando o tipo de criptografia e a chave do token
            SigningCredentials creds = new SigningCredentials(key, SecurityAlgorithms.HmacSha512Signature);

            //Instanciando a descrição do token para adicionar informações nele
            SecurityTokenDescriptor tokenDescriptor = new SecurityTokenDescriptor
            //Informações do Token
            {
                Subject = new ClaimsIdentity(claims),

                //Colocando o tempo para expirar
                Expires = DateTime.Now.AddDays(1),
                SigningCredentials = creds
            };

            //Intância que vai criar o token
